...
def parse_args():
    ...
    core.add_argument(
        ...
        default=None,
        metavar="LAST_PT",
        help="Resume training from last.pt checkpoint path. When set, --model and --weights are ignored.",
    )
    core.add_argument(
        "--data",
        type=str,
    ...
    sched.add_argument(
        "--fraction", type=float, default=1.0, help="Dataset fraction to use (0.0-1.0); useful for quick smoke tests"
    )
    sched.add_argument("--patience", type=int, default=100, help="Early stopping patience (epochs with no improvement)")
    sched.add_argument(
        "--save-period", type=int, default=-1, help="Save checkpoint every N epochs (-1 = only best/last)"
    )
    ...
    sched.add_argument(
        "--freeze", type=int, default=None, help="Freeze first N backbone layers (None = freeze nothing)"
    )
    ...
    loss.add_argument(
        "--kobj", type=float, default=1.0, help="Keypoint objectness loss weight (always 0 for face task)"
    )
    ...
    aug.add_argument(
        ...
        type=float,
        default=0.0,
        help="Copy-paste augmentation probability (not recommended for face — keypoints not handled)",
    )
    aug.add_argument(
        "--multi-scale",
        type=float,
        default=0.0,
        help="Multi-scale range as fraction of imgsz; sizes rounded to stride multiples",
    )
