...
class FaceModel(PoseModel):
    ...
    def init_criterion(self):
        """Initialize face loss criterion based on face_loss_type config.

        Routes:
            'auto' — Pose26 head → v8YOLOv6FaceLoss, Pose head → v8FaceLoss
            'v8'   — v8FaceLoss (WingLoss + BCE + CIoU, for YOLO11/12)
            'v6'   — v8YOLOv6FaceLoss (WingLoss + VFL + GIoU + RepulsionLoss, YOLOv6 clone)
            '26'   — FaceLoss26 (WingLoss + BCE + CIoU, Pose26 decode, YOLO26 native)
        """
        loss_type = getattr(self, "face_loss_type", "auto")
        head = self.model[-1]

        if loss_type == "v8":
            return E2ELoss(self, FaceLoss26) if getattr(self, "end2end", False) else v8FaceLoss(self)
        elif loss_type == "v6":
            return v8YOLOv6FaceLoss(self)
        elif loss_type == "26":
            return FaceLoss26(self)
        else:  # auto
